...
// Wrapped as an ES module function. Originally an n8n Code node script
// (which expects a magic `items` global) — converted in place so the
// Booking Form Esign app can render booking form HTML directly from the
// CRM's stored booking_forms.form_data JSON, no n8n in the loop.
//
// Call: renderBookingFormHtml(formData) → returns the HTML rows string
// (the same content the n8n version produced as { html: rows.join("") }).
// Caller wraps the returned string with the surrounding base.html template.

export function renderBookingFormHtml(formData) {
  const body = (formData && (formData.body || formData)) || {};
